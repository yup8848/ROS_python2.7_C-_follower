...
// 编解码转换表
unsigned char s_encTable[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
unsigned char s_decTable[] = {
...
// 分组编码
int __encode(unsigned char* pDest, const unsigned char* pSrc, size_t nSrcLen)
{
...
// 分组解码
int __decode(unsigned char* pDest, const unsigned char* pSrc, size_t nSrcLen)
{
	unsigned char * dst = pDest;
	const unsigned char * src = pSrc;
	size_t len = nSrcLen;
...
{
	std::string strEncode;
	strEncode.resize((nSrcLen / 3 + 1) * 4);
	strEncode.resize(__encode((unsigned char*)strEncode.data(), (const unsigned char*)pSrc, nSrcLen));
	return strEncode;
}
// 执行BASE64解码操作
...
{
	std::string strDecode;
	strDecode.resize(nSrcLen);
	strDecode.resize(__decode((unsigned char*)strDecode.data(), (const unsigned char*)pSrc, nSrcLen));
	return strDecode;
}
